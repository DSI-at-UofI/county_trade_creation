...
# Purpose: ----------------
# Here, we create the christian county flows to the rest of IL counties
...
dy_cnty <- dy_cnty %>% filter(orig_stName == "17021" & dest_stName == "illinois") %>%
...
ggplot() + 
  geom_polygon(data = USmap_cnty_df,
               aes(x = long,
                   y = lat,
                   group = group)) +
  geom_segment(data = dy_cnty %>% filter(cnty_flows != 0),
               aes(x = long_orig,
                   y = lat_orig,
                   xend = long_dest,
                   yend = lat_dest,
